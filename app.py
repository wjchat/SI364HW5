...
class UpdatePriority(FlaskForm):
    priority = IntegerField("What is the priority of this TODO item?", validators=[Required()])
...
def update(item):
    ...
    thisitem = TodoItem.query.filter_by(id = item).first()

    form = UpdatePriority()

    # if form.is_submitted():
    #     flash('submitted')
    #     flash(form.priority.data)
        # redirect(url_for('update', item = thisitem.id))
        # return render_template('update_item.html', form = form, item = thisitem)

    if form.validate_on_submit():
        # flash(form.priority.data)
        thisitem.priority = form.priority.data

        db.session.commit()

        flash('***the priority of ' + thisitem.description + ' has been updated***')

        return redirect(url_for('all_lists'))

    else:
        # flash('didnt validate')
        return render_template('update_item.html', form = form, item = thisitem)
...
def delete(lst):
    ...
    thislist = TodoList.query.filter_by(title = lst).first()

    # flash(lst)

    descr = thislist.title

    db.session.delete(thislist)
    db.session.commit()
